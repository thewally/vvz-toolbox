...
import { fetchAllMatchReports, deleteMatchReport, updateMatchReport } from '../services/matchReports'
...
export default function VerslagenBeheerPage() {
  ...
  async function load() {
    setLoading(true)
    const { data, error } = await fetchAllMatchReports()
  ...
  function isPublished(item) {
    return item.published_at && new Date(item.published_at) <= new Date()
  }

  async function handleTogglePublished(item) {
    const published = isPublished(item)
    const { error: updateError } = await updateMatchReport(item.id, {
  ...
  async function handleDelete(id) {
    const { error: deleteError } = await deleteMatchReport(id)
  ...
  return (
    <div className="max-w-5xl mx-auto p-4 pt-6">
      ...
      {items.length === 0 ? (
        <p className="text-gray-500 text-sm">Nog geen wedstrijdverslagen aangemaakt.</p>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100 text-left">
                ...
                <th className="px-5 py-3 font-medium text-gray-600 hidden sm:table-cell">Team</th>
              ...
              {items.map(item => {
                ...
                return (
                  <tr key={item.id}>
                    ...
                    <td className="px-5 py-3 text-gray-500 hidden sm:table-cell">{item.team_name}</td>
                    <td className="px-5 py-3 text-gray-500 hidden md:table-cell">
                      {item.published_at && new Date(item.published_at).getFullYear() < 9000
